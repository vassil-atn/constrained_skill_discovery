...
class SDPreTrainedPolicyAction(ActionTerm):
    ...
    cfg: SDPreTrainedPolicyActionCfg
    """The configuration of the action term."""

    def __init__(
        self, cfg: SDPreTrainedPolicyActionCfg, env: ManagerBasedRLEnv
    ) -> None:
        ...
        cfg.low_level_observations.actions.func = (
            lambda dummy_env: self.low_level_actions
        )
        cfg.low_level_observations.actions.params = dict()
        cfg.low_level_skills.selected_skills.func = (
            lambda dummy_env: 25
            * torch.abs(self._raw_actions[:, 2]).view(-1, 1)
            * (
                self._raw_actions[:, :2]
                / torch.norm(self._raw_actions[:, :2], dim=1, keepdim=True)
            )
        )
        cfg.low_level_skills.selected_skills.params = dict()

        # add the low level observations to the observation manager
        self._low_level_obs_manager = ObservationManager(
            {"ll_policy": cfg.low_level_observations, "skills": cfg.low_level_skills},
            env,
        )
    ...
    def apply_actions(self):
        if self._counter % self.cfg.low_level_decimation == 0:
            ...
            skills = self._low_level_obs_manager.compute_group("skills")
            # Scale up by the magnitude:
            # max_magn = self.cfg.max_magnitude
            # skills_magn = (self.cfg.scale_magnitude * torch.norm(skills, dim=1, keepdim=True)).clamp(0, max_magn)
            # skills = skills_magn * skills / torch.norm(skills, dim=1, keepdim=True)
            self.low_level_actions[:] = self.policy(
                torch.cat((low_level_obs, skills), dim=1)
            )[0]
...
@configclass
class SDPreTrainedPolicyActionCfg(ActionTermCfg):
    """Configuration for pre-trained policy action term.

    See :class:`PreTrainedPolicyAction` for more details.
    """

    class_type: type[ActionTerm] = SDPreTrainedPolicyAction
    ...
    low_level_skills: ObservationGroupCfg = MISSING
    """Low level skill configuration."""
    max_magnitude: float = MISSING
    """Maximum magnitude of the skills."""
    scale_magnitude: float = MISSING
    """Scale the magnitude of the skills."""
    debug_vis: bool = True
