...
The functions can be passed to the :class:`isaaclab.managers.RandomizationTermCfg` object to enable
...
def randomize_rigid_body_material(
    ...
    material_ids = torch.randint(
        0, num_buckets, (asset.root_physx_view.count, asset.root_physx_view.max_shapes)
    )
    materials = material_buckets[material_ids]
    # resolve the global body indices from the env_ids and the env_body_ids
    bodies_per_env = (
        asset.root_physx_view.count // num_envs
    )  # - number of bodies per spawned asset
...
def add_body_mass(
    ...
    # resolve the global body indices from the env_ids and the env_body_ids
    bodies_per_env = asset.root_physx_view.count // env.num_envs
    indices = torch.tensor(asset_cfg.body_ids, dtype=torch.int).repeat(len(env_ids), 1)
    indices += env_ids.unsqueeze(1) * bodies_per_env
...
def reset_root_state_uniform(
    ...
    * It samples the root velocity from the given ranges and sets them into the physics simulation.
    ...
    if env.initial_root_states.device != positions.device:
        env.initial_root_states = env.initial_root_states.to(positions.device)
    env.initial_root_states[env_ids] = torch.cat([positions, velocities], dim=-1)
