...
from skrl.agents.torch.exploration.entropy_ppo import EntropyPPO, PPO_DEFAULT_CONFIG
...
class Policy(GaussianMixin, Model):
    ...
    def compute(self, inputs, role):
        ...
        if torch.any(torch.isnan(mean_value)):
            print("mean_actions contains NaN values")
...
env = load_isaacgym_env_preview4(task_name="AnymalTerrain",headless=True,num_envs=4096)
env = wrap_env(env)
...
cfg["experiment"]["wandb_kwargs"] = {"project":"AnymalPPO",   
                        "mode":"online"}   
agent = EntropyPPO(models=models,
            memory=memory,
            cfg=cfg,
...
# path = "runs/torch/AnymalTerrain/24-01-22_17-36-50-605968_PPO/checkpoints/best_agent.pt"
# agent.load(path)
