...
class Agent:
    ...
    def init(self, trainer_cfg: Optional[Mapping[str, Any]] = None) -> None:
        ...
        if self.cfg.get("experiment", {}).get("wandb", False):
            ...
            # wandb_save_files = copy.deepcopy(self.cfg.get("experiment", {}).get("wandb_save_files", {}))
    ...
    def load_hierarchical_policies(
        self, path: str, level: str, eval: bool = False
    ) -> None:
        modules = torch.load(path, map_location=self.device)

        if type(modules) is dict:
            for name, data in modules.items():
                if name in ["policy", "value", "optimizer"]:
                    # Remap the module names -> policy in modules refers to the low-level policy
                    name = f"{level}_level_" + name
                ...
                if not eval and "high_level" in name:
                    continue

                if module is not None:
                    if hasattr(module, "load_state_dict"):
                        module.load_state_dict(data)
                        if eval and hasattr(module, "eval"):
                            module.eval()
                    else:
                        raise NotImplementedError
                else:
                    if modules[name] is None:
                        ...
                        raise "Cannot load the {name} module. The agent doesn't have such an instance"

    def load_for_training(self, path: str, eval: bool = False) -> None:
    ...
    def load(self, path: str, hierarchical: bool = False) -> None:
        """Load the model from the specified path
