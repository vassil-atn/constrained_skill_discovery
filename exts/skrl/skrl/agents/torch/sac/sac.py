...
from skrl.memories.torch.nstep_replay import NStepReplay
import collections
...
class SAC(Agent):
    def __init__(self,
        ...
        self.n_step_buffer = NStepReplay(observation_space.shape[0],
    ...
    def act(self, states: torch.Tensor, timestep: int, timesteps: int) -> torch.Tensor:
        ...
        if timestep < self._random_timesteps:
            return self.policy.random_act({"states": self._state_preprocessor(states,train=True)}, role="policy")
            # action = torch.rand((4096, 12),device=self.device) * 2.0 - 1.0
            # return action
        # sample stochastic actions
        actions, _, outputs = self.policy.act({"states": self._state_preprocessor(states,train=True)}, role="policy")
    ...
    def record_transition(self,
                          ...
                          next_curiosity_states: torch.Tensor = None) -> None:
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.no_grad():
                ...
                target_values = sampled_rewards + self._discount_factor**self.n_step * sampled_dones.logical_not() * target_q_values
            ...
            if self._learn_entropy:
                ...
                # Le = - (log_e * A)
                # self._entropy_coefficient.register_hook(lambda grad: print(grad))

                # print(self._entropy_coefficient.grad.data)
