...
class Memory:
    ...
    def sample_all(self, names: Tuple[str], mini_batches: int = 1, sequence_length: int = 1) -> List[List[torch.Tensor]]:
        ...
        if mini_batches > 1:
            ...
            indices = torch.randperm(self.memory_size * self.num_envs)
            batch_size=len(indices) // mini_batches
            # batches = BatchSampler(indexes, batch_size=len(indexes) // mini_batches, drop_last=True)
            ...
            for i in range(mini_batches):
                ...
                for key,value in self.tensors_view.items():
                    batch_data[key] = value[batch_idx]
