...
from skrl.memories.torch.simple_replay import create_buffer
...
class NStepReplay:
    ...
    def add_to_buffer(self, obs, actions, rewards, next_obs, dones):
        ...
        if self.nstep > 1:
            ...
            for i in range(1):
                self.nstep_buf_obs = self.fifo_shift(self.nstep_buf_obs, obs)
                self.nstep_buf_next_obs = self.fifo_shift(self.nstep_buf_next_obs, next_obs)
                self.nstep_buf_done = self.fifo_shift(self.nstep_buf_done, dones)
                self.nstep_buf_action = self.fifo_shift(self.nstep_buf_action, actions)
                self.nstep_buf_reward = self.fifo_shift(self.nstep_buf_reward, rewards)
                self.nstep_count += 1
                if self.nstep_count < self.nstep:
                    return obs, actions, rewards, next_obs, dones
                # we start at (s_5,s_6,s_7) and now we update the buf and we have (s_6,s_7,s_8). Then 
                # we compute the 3-step return for s_6 (till s_8) and next_obs is s_8.
                # Then this returns s_6, a_6, r_6, s_8, d_6 (i.e. the replay buffer is always 3 steps behind)
