...
from skrl.resources.torch_util import SquashedNormal,TanhNormal
...
class GaussianMixin:
    def __init__(self,
                 ...
                 max_log_std: float = 2,
                 reduction: str = "sum",
                 squashed_normal: bool = False,
                 normalize_output_as_unit: bool = False,
        ...
        if squashed_normal:
            reduction = "none"
            self._squashedNormal = True
        else:
            self._squashedNormal = False

        self._normalize_output_as_unit = normalize_output_as_unit
    ...
    def act_no_dict(self,inputs,role=""):

        # map from states/observations to mean actions and log standard deviations
        mean_actions, log_std, outputs = self.compute_no_dict(inputs)

        # clip actions
        if self._clip_actions:
            mean_actions = torch.clamp(mean_actions, min=self._clip_actions_min, max=self._clip_actions_max)

        mean_actions = torch.tanh(mean_actions) if self._squashedNormal else mean_actions
    ...
    def act(self,
        ...
        if "skills" in inputs:
            inputs["states"] = torch.cat([inputs["states"], inputs["skills"]], dim=-1)
        ...
        # distribution
        if self._squashedNormal:
            self._distribution = TanhNormal(mean_actions, log_std.exp())
            # alt = SquashedNormal(mean_actions, log_std.exp())
        else:
            self._distribution = Normal(mean_actions, log_std.exp())

        if self._normalize_output_as_unit:
            mean_actions = mean_actions / torch.norm(mean_actions, dim=-1, keepdim=True).clamp(min=1e-6)
            if self._squashedNormal:
                raise "Cannot normalize output as unit for squashed normal distribution."

        # sample using the reparameterization trick
        if self._squashedNormal:
            actions_pretanh, actions = self._distribution.rsample_with_pre_tanh_value()
        else:
            actions = self._distribution.rsample()
            actions_pretanh = 0*actions[:]
        ...
        if self._squashedNormal:
            log_prob = self._distribution.log_prob(inputs.get("taken_actions", actions),inputs.get("taken_actions_pretanh", actions_pretanh))
        else:
            log_prob = self._distribution.log_prob(inputs.get("taken_actions", actions))
        # Enforce action bound (from CleanRL):
        # log_prob = log_prob - torch.log((1 - actions_squashed.pow(2)) + 1e-6)
        if self._reduction is not None:
            log_prob = self._reduction(log_prob, dim=-1)
        if log_prob.dim() != actions.dim():
            log_prob = log_prob.unsqueeze(-1)

        outputs["mean_actions"] = torch.tanh(mean_actions) if self._squashedNormal else mean_actions
        # actions = actions_squashed
