...
def gaussian_model(observation_space: Optional[Union[int, Tuple[int], gym.Space, gymnasium.Space]] = None,
                   ...
                   squashed_normal: bool = False) -> Model:
    ...
    class GaussianModel(GaussianMixin, Model):
        def __init__(self, observation_space, action_space, device, clip_actions,
                     clip_log_std, min_log_std, max_log_std, reduction="sum", squashed_normal=False):
            Model.__init__(self, observation_space, action_space, device)
            GaussianMixin.__init__(self, clip_actions, clip_log_std, min_log_std, max_log_std, reduction, squashed_normal)
    ...
    metadata = {"input_shape": input_shape,
                ...
                "squashed_normal": squashed_normal}
    ...
    class GaussianDeterministicModel(GaussianMixin, DeterministicMixin, Model):
        def __init__(self, observation_space, action_space, device, roles, metadata):
            ...
            GaussianMixin.__init__(self,
                                   ...
                                   squashed_normal=metadata[0]["squashed_normal"],
                                   normalize_output_as_unit=metadata[0]["normalize_output_as_unit"],
                                   state_dependent_std=metadata[0]["state_dependent_std"],
                                   role=roles[0])
            ...
            self.state_dependent_std = metadata[0]["state_dependent_std"]

            if self.state_dependent_std:
                self.log_std_net = nn.Sequential(*[nn.Linear(metadata[0]["hiddens"][-1], _get_num_units_by_shape(self, metadata[0]["output_shape"]))])
        ...
        def compute(self, inputs, role):
            ...
            if role == self._roles[0]:
                if self.state_dependent_std:
                    return self.instantiator_output_scales[0] * self.mean_net(output), self.log_std_net(output), {}
                else:
        ...
        def compute_no_dict(self, inputs):
